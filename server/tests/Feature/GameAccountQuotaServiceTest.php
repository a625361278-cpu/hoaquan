...
class GameAccountQuotaServiceTest extends TestCase
{
    ...
    public function testExtraPointAddsOneMoreDayAndConsumesOneMorePoint(): void
    {
        $connection = Db::connection();
        $connection->beginTransaction();
        ...
        try {
            ...
            $result = $service->extendAccount($userId, $accountId, 1);

            $this->assertSame('9.00', (string)Db::table('ga_users')->where('id', $userId)->value('balance'));
            $this->assertSame('2026-07-30 15:58:51', (string)Db::table('ga_game_accounts')->where('id', $accountId)->value('expire_time'));
            $this->assertSame(11, $result['cost_points']);
            $this->assertSame(12, $result['add_days']);
        } finally {
            $connection->rollBack();
        }
    }
