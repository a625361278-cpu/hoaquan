...
`role_id` 是稳定唯一的游戏角色标识，用于平台用户绑定和邀请奖励去重，建议每次 `started` 都返回。允许常见账号/角色标识字符，最长 128 个字符，不能包含空白或控制字符。缺少 `role_id` 时，账号密码登录使用 `game_username`，Facebook/Google 登录使用 `game_uid` 作为绑定 ID。`display_name` 只用于展示角色名，不参与奖励和唯一身份判断。`started` 只确认真实运行并完成角色绑定，不直接发放邀请奖励。
...
普通日志按本次运行会话保存，最多保留 2500 条；主动停止和下次启动前会清空。结构化单条日志的 `time` 字段可传 `Y-m-d H:i:s` 字符串、10 位秒级时间戳或 13 位毫秒时间戳，推荐统一传 13 位毫秒时间戳；字段名仍为 `time`。`lines` 数组视为已格式化文本，服务端不会解析每行内部时间。空值或非法时间会使用服务器当前时间，并写入 warning 日志。普通日志会先进入 Redis 分片队列，由日志 writer 聚合后批量写库，用户端读取可能有几秒延迟。事件卡片历史只用于玩家游戏内事件展示，请优先使用结构化 `event` 消息；如果只能写在日志文本里，可在行尾追加 `[[EVT]]` 后接事件 JSON，服务端会解析并写入事件历史。连接断开、自动重连失败、配额到期停止等平台运行状态只写普通日志，不写事件卡片历史。
...
事件历史按游戏账号保存，跨停止/重启保留，最多保留 2500 条。`event.time` 与批量 `events` 中每条事件的 `time` 都可传 `Y-m-d H:i:s` 字符串、10 位秒级时间戳或 13 位毫秒时间戳，推荐统一传 13 位毫秒时间戳；服务端会统一转换为用户端展示用的 `Y-m-d H:i:s`。空值或非法时间会使用服务器当前时间，并写入 warning 日志。事件日志同样经过分片队列和 writer 批量写库，但刷新周期更短。用户点击清除事件历史时才会清空。`event` 应描述游戏内行为或收益结果，不用于平台连接、重连、到期停止等系统运行状态。
...
邀请奖励等级直接取当前绑定账号真实上报的 `status.level`。该账号必须已经收到有效 `started`、处于运行中，且账号角色与平台绑定角色一致；等级达到平台后台阈值后才可能奖励。奖励判断只接受 JSON 整数或纯数字字符串，缺失、负数、小数和非数字不会触发。`status` 与 `started` 的现有包体无需增加字段，也不要求第三方修改发送顺序。
