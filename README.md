...
- 用户端启动账号：`POST /api/game-accounts/{id}/start`。后端先校验账号配额未到期，再使用已保存的账号密码或 Token 发起一次与添加账号相同的第三方登录验证；验证通过后，用户端带 `validation_id` 再次请求同一接口，后端确认验证属于同一账号且凭证未变化，才解密对应凭证、读取本地配置 JSON、原子占用空闲脚本连接并发送 `start`。如果第三方验证回包在第一次接口返回前已经完成，服务端会直接消费本次验证结果并继续下发 `start`，避免账号停留在“验证成功但未真正启动”的中间态。如果该游戏账号已经处于 `starting/running/reconnecting` 且存在旧脚本绑定，会先完成启动前验证，再预留新空闲连接并向旧连接发送现有 `stop` 包，旧连接停止指令发送成功后才向新连接发送现有 `start` 包；启动方不弹窗，被顶掉的旧页面刷新或回到首页时提示“游戏帐号已经在别处登录”。`login_method=1` 只发送 `game_username + game_password`，`2/3` 只发送 `game_uid + token`；自动重连使用同一规则。第三方不需要新增字段，但必须继续支持现有登录验证消息；接口成功只表示启动包已发出，不表示第三方登录成功。
- 用户端账号处于 `starting/running/reconnecting/stopping` 时，首页“启动”按钮会置灰，避免运行中重复点击启动触发主动换绑流程；需要结束运行时必须点击“停止”。
...
- 新建配置或服务端配置缺少字段时会从 `client/src/utils/gameConfigSchema.js` 合并正式默认值；已经明确保存为 `false` 的账号保持关闭，不覆盖用户已有选择。当前默认开启的重点项包括 `basic.debug`（道具日志）、`basic.reconnectInterval=5`、任务配置的每日/每周/主线/剧情/花坊悬赏、自动祈愿、免费珍珠、雇佣劳工、水滴、解锁土地、种植自动收获、自动偷花、自动协助好友、居民订单、绸缎订单、顾客订单、宫廷订单、组团订单、公会土地自动收获和公会摸花。
